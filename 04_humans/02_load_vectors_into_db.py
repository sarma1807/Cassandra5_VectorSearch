...
try:
  ...
  cql_insert = cass_session.prepare("INSERT INTO faces (id, face_image, face_image_vector) VALUES (?, ?, ?)")
  ...
  for vfile in os.listdir(WEB_IMGS_FOLDER):
    ...
    if vfile.endswith(".jpg") and os.path.isfile(WEB_IMGS_FOLDER + '/' + vfile) and os.path.getsize(WEB_IMGS_FOLDER + '/' + vfile) > 0:
      ...
      face_image_vector = model.encode(Image.open(imgFile)).tolist()

      try:
        ### save the row in db
        cass_session.execute(cql_insert, (id, face_image, face_image_vector))
  ...
  ### all went well
  print("vector embeddings for human face images loaded into database.")
  print("Done.")
###############
