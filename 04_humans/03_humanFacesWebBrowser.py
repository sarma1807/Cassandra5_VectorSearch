...
class basicRequestHandler(tornado.web.RequestHandler):
  def get(self):
    vColumnCount = 0
    self.write("<h2> <a href='./form'> Click here to run Vector Search </a> </h2> \n")
    self.write("<h3> Basic Grid of Human Faces </h3> \n")
...
class formRequestHandler(tornado.web.RequestHandler):
  def get(self):
    vColumnCount = 0
    self.write("<h3> Click on a Human Face Image to perform ANN similarity search ... </h3> <br> \n")
...
class formClickedRequestHandler(tornado.web.RequestHandler):
  def get(self):
    ...
    cql_cars_ann_search = cass_session.prepare("SELECT face_image FROM faces ORDER BY face_image_vector ANN OF ? LIMIT 10")
    ...
    self.write("<table border=0> <tr> <td> <h3> input human face </h3> </td> <td> <img src='" + vImgFile + "' width='40%' height='40%'> <tr> </table> <br> <hr> \n")
    self.write("<h3> 10 human faces similar to the input face (search results using Cassandra 5.x ANN Vector Search) </h3> <br> \n")
    self.write("<table border=0> <tr> \n")

    for cass_row in cass_output_1:
      if vColumnCount % 5 == 0:
        self.write("</tr> <tr> \n")
      self.write("<td> <img src='" + WEB_IMGS_FOLDER + "/" + cass_row.face_image + "' width='40%' height='40%'> </td> \n")
      vColumnCount = vColumnCount + 1
    self.write("</tr> </table> \n")
